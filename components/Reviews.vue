...
// Fetch approved reviews when the component is mounted
onMounted(async () => {
  try {
    const allReviews = await $fetch('/api/reviews/get');
    reviews.value = allReviews.filter((review) => review.isApproved); // Only display approved reviews
  } catch (error) {
